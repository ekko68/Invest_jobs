...
const RequestInvestorControlTab = forwardRef((props, ref) => {
  ...
  const handleTabList = (id) => {
    ...
    if (id === 'receive') {
      history.push(ROUTER_NAMES.MY_PAGE_INVESTOR_REQUEST_RECEIVE_VIEW)
    } else {
      history.push(ROUTER_NAMES.MY_PAGE_INVESTOR_REQUEST_SEND_VIEW)
    }
  }
  ...
  const loadReceive = async () => {
    ...
    const receiveObject = await ResponseUtils.getSimpleResponse(Api.my.vc.auditReceiveList, params, false)
    if (receiveObject) {
  ...
  const loadSend = async () => {
    ...
    const sendObject = await ResponseUtils.getSimpleResponse(Api.my.vc.auditSendList, params, false)
    if (sendObject) {
      return sendObject
    }
    return null
...
export default RequestInvestorControlTab;
