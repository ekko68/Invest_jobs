...
const RecommendEmployeePopup = (props) => {
  const { activeOption , handlePopup, setEmp,brcd  } = props
  ...
  const search = async () => {
    ...
    if (!StringUtils.hasLength(brcd)) {
      setAlertPop({...alertPop , text : '추천 영업점을 먼저 선택해 주세요.' , active : true})
      console.log('test')
      return
    }
    ...
    const res = await CommonAxios('IVT', getPostConfig(Api.invest.ibkEmpList, { brcd: brcd, empNm: searchNm }))
        ...
        <div className="popup_wrap">
          ...
          <div className="popup_container" style={{height : '400px', width : '500px'}}>
            <div className="popup_main_header">
              <div className="title">추천 직원 조회</div>
              <Button
              ...
              <div className="cnt_wrap" style={{ width: '100%', height: 'auto !important' }}>
                ...
                <div className="reco_search_wrap">
                  {searchList?.length > 0 ? (
                    ...
                    <div className="reco_search">
                      <br/>
                      <div className="reco_search_nodata">일치하는 직원이 없습니다.</div>
                    </div>
                  )}
                </div>
              </div>
...
export default RecommendEmployeePopup
