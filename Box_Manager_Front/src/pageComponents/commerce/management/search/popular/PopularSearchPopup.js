...
const PopularSearchPopup = (props) => {
  const { onClosePopup, initialEditId, popularSearchData } = props
  ...
  //상품저장
  const handleSave = () => {
    const popularCnt = popularSearchData.list.filter(
      (item) => item.oppbYn === 'Y' && item.srwdInfId !== data.srwdInfId
    ).length
    if (data.oppbYn === 'Y' && popularCnt >= 5) {
      mktContext.actions.setCommonAlertInfo({
        type: 'alert',
        active: true,
      ...
      <div className="popup_container">
        <div className="popup_content scroll">
          <div className="popup_main_header">
            <div className="title">인기 검색어 관리</div>
          ...
          {tabList.active === 'productRegistration' ? (
            <div className="tabContents" id="tab_productRegistration">
              ...
              />
              <div className="popup_title">인기 검색어 정보</div>
...
export default PopularSearchPopup
