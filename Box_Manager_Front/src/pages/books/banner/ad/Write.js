...
import {getBannerAdDetail, saveBannerAd} from 'modules/consts/BooksApi'
...
const Write = () => {
      ...
      if(res.data.code === "200"){
        ...
        setUploadData({
          ...
          imgFileInfo: {
            ...
            fileEtns:  res.data.data.pcFileEtns,
  ...
  const handleValidate = () => {
    ...
    if(params?.bltddSttgDt?.length < 6 || params?.bltddFnshDt?.length < 6 ||
      params?.bltddSttgDt === "Invalid date" || params?.bltddFnshDt === "Invalid date") {
  ...
  const handleSave = async () => {
    let params = bannerForm;
    params["bnnrImgFileId"] = uploadData.fileId;
    ...
    let res = await saveBannerAd(params);
    if(res.data.code === "200"){
    ...
    <PageLayout currentMenu={'books'} currentCate={'booksBanner'} currentPage={'booksBannerAd'}>
      ...
      {popup.active && popup.type === "confirm" && (
        <PopupAlert msg={popup.text}
                    handlePopup={() => history.push(`${ROUTER_NAMES.BOOKS_BANNER_AD_VIEW}/${saveData.bnnrSqn}`)} />
      )}
      ...
      <div className='content_inner page_books books_write'>
        <h4 className='page_title'>광고배너 {id ? "수정" : "등록"}</h4>
                ...
                <td>
                  <div className="date_input_wrap ico_type02">
                    <div className="date_inputs">
                      <DatePicker
                        selected={bannerForm.bltddSttgDt ? moment(bannerForm.bltddSttgDt, "YYYYMMDD").valueOf() : null}
                        maxDate={bannerForm.bltddFnshDt ? moment(bannerForm.bltddFnshDt, "YYYYMMDD").valueOf() : null}
                        locale={ko}
                        dateFormat={'yyyy-MM-dd'}
                        title={'기간'}
                        onChange={(date) => setBannerForm({...bannerForm, bltddSttgDt: moment(date).format("YYYYMMDD")})}
                      />
                      <span> ~ </span>
                      <DatePicker
                        selected={bannerForm.bltddFnshDt ? moment(bannerForm.bltddFnshDt, "YYYYMMDD").valueOf() : null}
                        minDate={bannerForm.bltddSttgDt ? moment(bannerForm.bltddSttgDt, "YYYYMMDD").valueOf() : null}
                        locale={ko}
                        dateFormat={'yyyy-MM-dd'}
                        title={'기간'}
                        onChange={(date) => setBannerForm({...bannerForm, bltddFnshDt: moment(date).format("YYYYMMDD")})}
                      />
                    </div>
                  </div>
                </td>
              ...
              <tr>
                ...
                <td>
                  ...
                  <p className="notice highlight_red require" style={{ margin: '10px 0 10px 10px', fontSize: '13px' }}>
                    광고배너 적정사이즈는 가로 388px / 세로 262px 입니다.
                  </p>
                </td>
              </tr>
              <tr>
