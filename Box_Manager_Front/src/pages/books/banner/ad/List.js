...
import {getBannerAdList, saveBannerAdOrder, deleteBannerAd} from 'modules/consts/BooksApi'
...
const List = () => {
  ...
  useEffect(async() => {
    //리스트 정렬 순서 바뀔때마다 저장
    await saveBannerAdOrder(bannerList);
  }, [bannerList])

  const getBannerList = async () => {
    let res = await getBannerAdList();
  ...
  const handleButtons = (type, item) => {
    switch (type){
      case "add":
        ...
        navigatePage(`${ROUTER_NAMES.BOOKS_BANNER_AD_EDIT}`);
        break;
      case "edit":
        navigatePage(`${ROUTER_NAMES.BOOKS_BANNER_AD_EDIT}/${item.bnnrSqn}`);
        break;
      case "delete":
    ...
    <PageLayout currentMenu={'books'} currentCate={'booksBanner'} currentPage={'booksBannerAd'}>
      ...
      <div className='content_inner page_books'>
        <div className="page_header">
          <h4 className="page_title">광고배너</h4>
        ...
        <div className={'section banner_ad_section'}>
          {
            bannerList && bannerList.length > 0 ? (
              <div className="table_wrap border_bottom_none table_th_border banner_list_table scroll">
                <div className="custom_table apply_prod_table">
                  ...
                  <ReorderBanner type={"ad"}
                                 list={bannerList} setList={setBannerList}
                                 handleButtons={handleButtons}
                                 handleMoveDetail={(item) => navigatePage(`${ROUTER_NAMES.BOOKS_BANNER_AD_VIEW}/${item.bnnrSqn}`)}/>
                </div>
              </div>
            ) : (
