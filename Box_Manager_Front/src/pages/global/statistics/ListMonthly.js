...
const List = () => {
  ...
  const getList = async (params) => {
    ...
    if (params?.searchFromDate || params?.searchFromDate) {
      _searchFromDate = moment(params.searchFromDate).format('YYYYMM')
      _searchToDate = moment(params.searchToDate).format('YYYYMM')
    } else {
      _searchFromDate = moment(searchDate.startDate).format('YYYYMM')
      _searchToDate = moment(searchDate.endDate).format('YYYYMM')
    }
    ...
    let res = await getConsultStatisticsMonthly(paramsList) // 목록
  ...
  const hanldeExcelDownload = async () => {
    loader(true, 'Uploading...')
    let _startDate = moment(searchDate.startDate).format('YYYYMM')
    let _endDate = moment(searchDate.endDate).format('YYYYMM')
    let paramsList = {
      page: paging && paging.page ? paging.page : 1,
      svcDsnc: 'GLOBALBIZ',
      stDt: _startDate,
      enDt: _endDate,
      menuAccessType: '002' // 001 : Monthly
    }
  ...
  return (
    <PageLayout currentMenu={'global'} currentCate={'globalConsult'} currentPage={'globalStatisticsMonthList'}>
      <div className="content_inner page_static">
        ...
        <div className="static_table">
          <table className="table type02">
            <caption>월별 접속이력 현황 테이블</caption>
            <colgroup>
              <col width={'40%'} />
              <col width={'*'} />
            </colgroup>
