...
import { getServiceEventDetail, saveServiceEvent} from 'modules/consts/MainApi'
...
const Write = () => {
  ...
  const handleSave = async () => {
    ...
    let res = await saveServiceEvent(params);
    if(res.data.code === "200"){
  ...
  return (
    <PageLayout currentMenu={'mainBox'} currentCate={'mainServiceMenu'} currentPage={'mainServiceEvent'}>
      ...
      {popup.active && popup.type === "confirm" && (
        <PopupAlert
          msg={popup.text}
          handlePopup={() => history.push(`${ROUTER_NAMES.MAIN_SERVICEMENU_EVENT_VIEW}/${saveData.evntSvcId}`)}
        />
      )}
            ...
            <tr>
              ...
              <td>
                <input
                  ...
                  title={'메인타이틀'}
                  value={serviceForm.evntSvcTtl}
                  maxLength={38}
                  onChange={(e) => handleText("evntSvcTtl", e.target.value)}
                />
              </td>
            </tr>
            <tr>
