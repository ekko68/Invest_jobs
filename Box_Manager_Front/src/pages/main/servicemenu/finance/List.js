...
import {
  deleteServiceFinance,
  getServiceFinanceList,
...
const List = () => {
  ...
  const apiGetFinanceList = async () => {
    let res = await getServiceFinanceList(mainContext.state.serviceListParam); // 서비스(금융서비스)
    if(res.data?.code === "200"){
      let data = res.data.data;
      setFinanceList(data.list);
      setPaging({
  ...
  const handleButtons = (type, item) => {
    switch (type){
      case "add":
        navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_FINANCE_WRITE}`);
        break;
      case "edit":
        navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_FINANCE_WRITE}/${item.fnncSvcId}`);
        break;
      case "delete":
  ...
  const handleDelete = async() => {
    let res = await deleteServiceFinance({
      updateItem: delParam?.fnncSvcId, //id
    ...
    <PageLayout currentMenu={'mainBox'} currentCate={'mainServiceMenu'} currentPage={'mainServiceFinance'}>
      ...
      <div className='content_inner page_main'>
        <div className="page_header">
          <h4 className="page_title">금융서비스</h4>
                  ...
                  <div className="t_body">
                    {
                      financeList.map((item) => (
                        <div className="item-container" key={item.fnncSvcId}
                             onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_FINANCE_VIEW}/${item.fnncSvcId}`)}>
                          <div className="t_row">
                            ...
                            <div className="t_cell">
                              <div className="ellipsis2">{item?.fnncSvcTtl || "-"}</div>
                            </div>
