...
import {deleteServiceFinance, getServiceFinanceDetail} from 'modules/consts/MainApi'
    ...
    <PageLayout currentMenu={'mainBox'} currentCate={'mainServiceMenu'} currentPage={'mainServiceFinance'}>
      ...
      {popup.active && popup.type === "confirm" && (
        <PopupAlert msg={'해당 게시글이 삭제되었습니다.'}
                    handlePopup={() => navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_FINANCE_LIST}`)} />
      )}
      ...
      <div className='content_inner page_main service_view'>
        <h4 className='page_title'>금융서비스 상세</h4>
        ...
        <div className="table_wrap">
          <table className="table_type02">
            ...
            <tr>
              ...
              <td>{serviceData?.fnncSvcTtl || "-"}</td>
            </tr>
            <tr>
              <th className={'ta_left'}>이미지등록</th>
              <td>
                <div className="img_wrap_service">
                  {serviceData?.bltImgUrl && <img src={serviceData.bltImgUrl} alt={serviceData?.bnfsSvcId} />}
                </div>
        ...
        <div className="rounded_btn_group">
          <Button className={'basic'} onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_FINANCE_LIST}`)}>
            목록
          </Button>
          <Button className={'full_blue'} onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_FINANCE_WRITE}/${id}`)}>
            수정
          </Button>
