...
import {deleteServiceBenefit, getServiceBenefitDetail} from 'modules/consts/MainApi'
    ...
    <PageLayout currentMenu={'mainBox'} currentCate={'mainServiceMenu'} currentPage={'mainServiceBenefits'}>
      ...
      {popup.active && popup.type === "confirm" && (
        <PopupAlert msg={'해당 게시글이 삭제되었습니다.'}
                    handlePopup={() => navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_BENEFITS_LIST}`)} />
      )}
      ...
      <div className='content_inner page_main service_view'>
        <h4 className='page_title'>유용한혜택 상세</h4>
        ...
        <div className="rounded_btn_group">
          <Button className={'basic'} onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_BENEFITS_LIST}`)}>
            목록
          </Button>
          <Button className={'full_blue'} onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_BENEFITS_WRITE}/${id}`)}>
            수정
          </Button>
