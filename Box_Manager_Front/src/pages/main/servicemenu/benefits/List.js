...
const List = () => {
  ...
  const apiGetBenefitsList = async () => {
    let res = await getServiceBenefitList(mainContext.state.serviceListParam); // 서비스(유용한헤택)
    if(res.data?.code === "200"){
      let data = res.data.data;
      setBenefitsList(data.list);
      setPaging({
  ...
  const handleButtons = (type, item) => {
    switch (type){
      case "add":
        navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_BENEFITS_WRITE}`);
        break;
      case "edit":
        navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_BENEFITS_WRITE}/${item.bnfsSvcId}`);
        break;
      case "delete":
  ...
  const handleDelete = async() => {
    let res = await deleteServiceBenefit({
      updateItem: delParam?.bnfsSvcId, //id
    ...
    <PageLayout currentMenu={'mainBox'} currentCate={'mainServiceMenu'} currentPage={'mainServiceBenefits'}>
      ...
      <div className='content_inner page_main'>
        <div className="page_header">
          <h4 className="page_title">유용한 혜택</h4>
                  ...
                  <div className="t_body">
                    {
                      benefitsList.map((item) => (
                        <div className="item-container" key={item.bnfsSvcId}
                             onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_SERVICEMENU_BENEFITS_VIEW}/${item.bnfsSvcId}`)}>
                          <div className="t_row">
                            ...
                            <div className="t_cell">
                              <div className="ellipsis2">{item?.bnfsSvcTtl || "-"}</div>
                            </div>
