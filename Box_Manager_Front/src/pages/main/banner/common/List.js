...
const List = () => {
  ...
  const apiGetBannerList = async () => {
    let res = await getBannerList({bnnrComCdId: "BNR00004"}); //배너코드(PC화면배너)
    if(res.data?.code === "200"){
      let setBannerListForm = res.data.data?.list?.map((item) => {
        return {...item, bnnrComCdId: "BNR00004"}; //배너코드(PC화면배너)
      })
  ...
  const handleButtons = (type, item) => {
    switch (type){
      case "add":
        ...
        navigatePage(`${ROUTER_NAMES.MAIN_BANNER_COMMON_WRITE}`);
        break;
      case "edit":
        navigatePage(`${ROUTER_NAMES.MAIN_BANNER_COMMON_WRITE}/${item.bnnrSqn}`);
        break;
      case "delete":
  ...
  const handleDelete = async() => {
    let res = await deleteBanner({
      bnnrComCdId: "BNR00004", //배너코드(PC화면배너)
  ...
  return (
    <PageLayout currentMenu={'mainBox'} currentCate={'mainBanner'} currentPage={'mainBannerCommon'}>
      ...
      <div className='content_inner main_banner_list'>
        <div className="page_header">
          <h4 className="page_title">PC화면배너(우측)</h4>
              ...
              <div className="table_wrap border_bottom_none table_th_border biz_list_table scroll">
                <div className="custom_table apply_prod_table">
                  ...
                  <ReorderBanner list={bannerList} setList={setBannerList}
                                 ...
                                 handleMoveDetail={(item) => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_COMMON_VIEW}/${item.bnnrSqn}`)}/>
                </div>
              </div>
            ) : (
