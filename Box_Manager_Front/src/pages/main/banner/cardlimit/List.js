...
const List = () => {
  ...
  const apiGetBannerList = async () => {
    let res =  await getBannerList({bnnrComCdId: "BNR00002"}); //배너코드(카드한도조회)
    if (res.data?.code === '200') {
      let setBannerListForm = res.data.data?.list?.map((item) => {
        return {...item, bnnrComCdId: "BNR00002"}; //배너코드(카드한도조회)
      })
  ...
  const handleButtons = (type, item) => {
    switch (type) {
      case "add" :
        ...
        navigatePage(`${ROUTER_NAMES.MAIN_BANNER_CARDLIMIT_WRITE}`);
        break;
      case "edit" :
        navigatePage(`${ROUTER_NAMES.MAIN_BANNER_CARDLIMIT_WRITE}/${item.bnnrSqn}`);
        break
      case "delete" :
    ...
    <PageLayout currentMenu={'mainBox'} currentCate={'mainBanner'} currentPage={'mainBannerCardLimit'}>
      ...
      <div className='content_inner main_banner_list'>
        <div className="page_header">
          <h4 className="page_title">카드한도조회(하단)</h4>
              ...
              <div className="table_wrap border_bottom_none table_th_border biz_list_table scroll">
                ...
                <div className="custom_table apply_prod_table">
                  ...
                  <ReorderBanner
                    ...
                    handleMoveDetail={(item) => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_CARDLIMIT_VIEW}/${item.bnnrSqn}`)}/>
                </div>
                {/*custom_table end*/}
              </div>
            ) : (
