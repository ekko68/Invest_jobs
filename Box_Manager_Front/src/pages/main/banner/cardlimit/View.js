...
const View = () => {
  ...
  useEffect(async() => {
    if(id) {
      let res = await getBannerDetail({bnnrComCdId: "BNR00002", bnnrSqn: id}); //배너코드(카드한도조회)
  ...
  const handleDelete = async() => {
    let res = await deleteBanner({
      bnnrComCdId: "BNR00002", //배너코드(카드한도조회)
  ...
  return (
    <PageLayout currentMenu={'mainBox'} currentCate={'mainBanner'} currentPage={'mainBannerCardLimit'}>
      ...
      {popup.active && popup.type === "confirm" && (
        <PopupAlert msg={'해당 배너가 삭제되었습니다.'}
                    handlePopup={() => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_CARDLIMIT_LIST}`)} />
      )}
      ...
      <div className='content_inner page_books_main'>
        <h4 className='page_title'>카드한도조회(하단) 배너 상세</h4>

        <div className="table_wrap">
          <table className="table_type02">
            <caption>카드한도조회(하단) 배너 상세 테이블</caption>
            <colgroup>
              <col width={'13%'} />
              <col width={'*'} />
            </colgroup>
        ...
        <div className="rounded_btn_group">
          <Button className={'basic'} onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_CARDLIMIT_LIST}`)}>
            목록
          </Button>
          <Button className={'full_blue'} onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_CARDLIMIT_WRITE}/${bannerData.bnnrSqn}`)}>
            수정
          </Button>
