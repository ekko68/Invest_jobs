    ...
    <PageLayout currentMenu={'mainBox'} currentCate={'mainBanner'} currentPage={'mainBannerMore'}>
      ...
      {popup.active && popup.type === "confirm" && (
        <PopupAlert msg={popup.text}
                    handlePopup={() => history.push(`${ROUTER_NAMES.MAIN_BANNER_MORE_VIEW}/${saveData.bnnrSqn}`)} />
      )}
      ...
      <div className='content_inner pages_main main_write'>
        <h4 className='page_title'>더보기(중간) 배너 {id ? "수정" : "등록"}</h4>


        <div className="table_wrap">
          <table className="table form_table bg_none border_none">
            <caption>더보기(중간) 배너 테이블</caption>
            <colgroup>
              <col width={'13%'} />
              <col width={'*'} />
            </colgroup>
