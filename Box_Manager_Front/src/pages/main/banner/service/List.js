...
const List = () => {
  ...
  const apiGetBannerList = async () => {
    let res = await getBannerList({bnnrComCdId: "BNR00003"}); //배너코드(서비스)
    if (res.data?.code === '200'){
      let setBannerListForm = res.data.data?.list?.map((item) => {
        return {...item, bnnrComCdId: "BNR00003"}; //배너코드(서비스)
      })
  ...
  const handleButtons = (type, item) => {
    switch (type) {
      case 'add' :
        ...
        navigatePage(`${ROUTER_NAMES.MAIN_BANNER_SERVICE_WRITE}`);
        break;
      case 'edit' :
        navigatePage(`${ROUTER_NAMES.MAIN_BANNER_SERVICE_WRITE}/${item.bnnrSqn}`)
        break;
      case 'delete' :
    ...
    <PageLayout currentMenu={'mainBox'} currentCate={'mainBanner'} currentPage={'mainBannerService'}>
      ...
      <div className='content_inner main_banner_list'>
        <div className="page_header">
          <h4 className="page_title">서비스(상단)</h4>
              ...
              <div className="table_wrap border_bottom_none table_th_border biz_list_table scroll">
                <div className="custom_table apply_prod_table">
                  ...
                  <ReorderBanner list={bannerList} setList={setBannerList}
                                 ...
                                 handleMoveDetail={(item) => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_SERVICE_VIEW}/${item.bnnrSqn}`)}/>
                </div>
              </div>
            ) : (
