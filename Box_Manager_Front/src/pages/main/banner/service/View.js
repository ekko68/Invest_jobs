    ...
    <PageLayout currentMenu={'mainBox'} currentCate={'mainBanner'} currentPage={'mainBannerService'}>
      ...
      {popup.active && popup.type === "confirm" && (
        <PopupAlert msg={'해당 배너가 삭제되었습니다.'}
                    handlePopup={() => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_SERVICE_LIST}`)} />
      )}
      ...
      <div className='content_inner page_books_main'>
        <h4 className='page_title'>서비스(상단) 배너 상세</h4>

        <div className="table_wrap">
          <table className="table_type02">
            <caption>서비스(상단) 배너 상세 테이블</caption>
            <colgroup>
              <col width={'13%'} />
              <col width={'*'} />
            </colgroup>
        ...
        <div className="rounded_btn_group">
          <Button className={'basic'} onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_SERVICE_LIST}`)}>
            목록
          </Button>
          <Button className={'full_blue'} onClick={() => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_SERVICE_WRITE}/${bannerData.bnnrSqn}`)}>
            수정
          </Button>
