...
const List = () => {
  ...
  const apiGetBannerList = async () => {
    let res = await getBannerList({bnnrComCdId: "BNR00000"}); //배너코드(거래처 채팅)
    if(res.data?.code === "200"){
      let setBannerListForm = res.data.data?.list?.map((item) => {
        return {...item, bnnrComCdId: "BNR00000"}; //배너코드(거래처 채팅)
      })
  ...
  const handleButtons = (type, item) => {
    switch (type){
      case "add":
        ...
        navigatePage(`${ROUTER_NAMES.MAIN_BANNER_BUSINESSCHAT_WRITE}`);
        break;
      case "edit":
        navigatePage(`${ROUTER_NAMES.MAIN_BANNER_BUSINESSCHAT_WRITE}/${item.bnnrSqn}`);
        break;
      case "delete":
  ...
  const handleDelete = async() => {
    let res = await deleteBanner({
      bnnrComCdId: "BNR00000", //배너코드(거래처 채팅)
  ...
  return (
    <PageLayout currentMenu={'mainBox'} currentCate={'mainBanner'} currentPage={'mainBannerBizChat'}>
      ...
      {popup.active && popup.type === "over" && (
        <PopupAlert msg={'배너는 최대 5개까지 등록이 가능합니다.'}
      ...
      <div className='content_inner main_banner_list'>
        <div className="page_header">
          <h4 className="page_title">거래처 채팅(상단)</h4>
            ...
            bannerList && bannerList.length > 0 ? (
              <div className="table_wrap border_bottom_none table_th_border biz_list_table scroll">
                <div className="custom_table apply_prod_table">
                  ...
                  <ReorderBanner list={bannerList} setList={setBannerList}
                                 handleButtons={handleButtons}
                                 handleMoveDetail={(item) => navigatePage(`${ROUTER_NAMES.MAIN_BANNER_BUSINESSCHAT_VIEW}/${item.bnnrSqn}`)}/>
                </div>
              </div>
            ) : (
