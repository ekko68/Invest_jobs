...
const Agency = () => {
  ...
  const [searchFilter, setSearchFilter] = useState({
    ...
    list: [
      { id: 'bplcNm', value: 'bplcNm', label: '에이전시 업체명' },
  ...
  // ===== getList
  const getList = async () => {
    let res = await getPriceAgencyList(paramList)
    ...
    <PageLayout currentMenu={'commerce'} currentCate={'price'} currentPage={'agency'}>
      <div className="content_inner page_price">
        <div className="page_header">
          <h4 className="page_title">에이전시 별 총 판매 금액</h4>
        ...
        {/*section start*/}
        <div className="section price_agency_section">
          {!list || list.length === 0 ? (
            ...
            <div className="table_wrap border_bottom_none table_th_border">
              <table className="table type02">
                <caption>에이전시 별 총 판매 금액 테이블</caption>
                <colgroup>
                  <col width={'5%'} />
                  <col width={'*'} />
                  <col width={'13%'} />
                ...
                <tbody>
                  {list?.map((item, idx) => (
                    <tr key={commonFn.createKey()}>
                      ...
                      <td className={'ta_center'}>
                        {item.agenArlCnt ? commonFn.krwFormatter(item.agenArlCnt) : '0'}개
                      </td>
