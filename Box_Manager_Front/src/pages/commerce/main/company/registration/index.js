...
const Registration = (props) => {
  ...
  const handleSetConData = (type, value) => {
    setConData({
      ...conData,
      [type]: value
    })
  ...
  const handleSaveCb = () => {
    mktContext.actions.setCommonAlertInfo({
      ...
      msg: '저장되었습니다.',
      action: () => {
        history.push(`${ROUTER_NAMES.COMMERCE_MAIN_COMPANY}`, { webYn: mktContext.state.currType })
      }
    })
  }
  ...
  const handleUpdateDataSet = async () => {
    ...
    if (res.data.code === '200') {
      ...
      let conData = ''
      if (commonFn.isInvalidJSON(data.con)) {
        conData = JSON.parse(data.con)
      }
      setData({
      ...
      if (conData) {
        setConData({
          ttlTxt1: conData.ttlTxt1,
          ttlTxt2: conData.ttlTxt2,
          ttlTxt3: conData.ttlTxt3,
          conTxt1: conData.conTxt1,
          conTxt2: conData.conTxt2,
          conTxt3: conData.conTxt3
        })
      }
      await loader()
    }
  ...
  return (
    <PageLayout currentMenu={'commerce'} currentCate={'main'} currentPage={'companyList'}>
      <div className="content_inner page_main">
        <div className="page_header">
          <h4 className="page_title">{`기업 메인 (${
              ...
              <tr>
                ...
                <td>
                  <TextInput
                    ...
                    value={conData.ttlTxt1}
                    length={15}
                    onChange={(v) => {
                      handleSetConData('ttlTxt1', v)
                    }}
                  />
                  <TextInput
                    title="배너 제목"
                    placeholder="배너 제목을 입력하세요."
                    value={conData.ttlTxt2}
                    length={15}
                    onChange={(v) => {
                      handleSetConData('ttlTxt2', v)
                    }}
                  />
                  <TextInput
                    title="배너 제목"
                    placeholder="배너 제목을 입력하세요."
                    value={conData.ttlTxt3}
                    length={15}
                    onChange={(v) => {
                      handleSetConData('ttlTxt3', v)
                    }}
                  />
                </td>
              </tr>
              ...
              <tr>
                ...
                <td>
                  <TextInput
                    ...
                    value={conData.conTxt1}
                    length={30}
                    onChange={(v) => {
                      handleSetConData('conTxt1', v)
                    }}
                  />
                  <TextInput
                    title="배너 설명"
                    placeholder="설명을 입력하세요."
                    value={conData.conTxt2}
                    length={30}
                    onChange={(v) => {
                      handleSetConData('conTxt2', v)
                    }}
                  />
                  <TextInput
                    title="배너 설명"
                    placeholder="설명을 입력하세요."
                    value={conData.conTxt3}
                    length={30}
                    onChange={(v) => {
                      handleSetConData('conTxt3', v)
                    }}
                  />
                </td>
              </tr>
