...
const InvestOffer = () => {
  ...
  const getList = async (params) => {
    ...
    // 목록
    let res = await getStatisticsList('suggest', paramsList)
    // 합계
    let total = await getStatisticsTotal('suggest', paramsList)
  ...
  const hanldeExcelDownload = async () => {
    ...
    let res = await excelDownloadIVT('suggest', _startDate, _endDate)
  }
  ...
  return (
    <PageLayout currentMenu={'invest'} currentCate={'static'} currentPage={'investOffer'}>
      <div className="content_inner page_static">
        <h4 className="page_title">진행 중인 투자심사제안</h4>
        ...
        {searchDate && (
          <div className="search_info">
            ...
            <span className="end_date">{dateFormatUnitSplit(searchDate.endDate)}</span> 투자심사제안
          </div>
        )}
        {/*static_table start*/}
        <div className="static_table ">
          <table className="table type02">
            <caption>진행 중인 투자심사제안 테이블</caption>
            <colgroup>
              <col width={'20%'} />
              <col width={'20%'} />
              <col width={'20%'} />
