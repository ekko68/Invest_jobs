...
const InvestComp = () => {
  ...
  const getList = async (params) => {
    ...
    // 목록
    let res = await getStatisticsList('complete', paramsList)
    // 합계
    let total = await getStatisticsTotal('complete', paramsList)
  ...
  const hanldeExcelDownload = async () => {
    ...
    let res = await excelDownloadIVT('complete', _startDate, _endDate)
  }
  ...
  return (
    <PageLayout currentMenu={'invest'} currentCate={'static'} currentPage={'investComp'}>
      <div className="content_inner page_static">
        <h4 className="page_title">투자심사완료</h4>
        ...
        {searchDate && (
          <div className="search_info">
            ...
            <span className="end_date">{dateFormatUnitSplit(searchDate.endDate)}</span> 투자심사완료
          </div>
        )}
        {/*static_table start*/}
        <div className="static_table ">
          <table className="table type02">
            <caption>진행 중인 투자심사완료 테이블</caption>
            <colgroup>
              <col width={'20%'} />
              <col width={'20%'} />
              <col width={'20%'} />
                ...
                <th className={'ta_right'} colSpan={5}>
                  <div className="total_wrap">
                    <dl className="total dl">
                      <dt>총</dt>
                      <dd>{total ? commonFn.krwFormatter(total) : 0} 건</dd>
                    </dl>
