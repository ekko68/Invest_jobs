...
const MuiNumberInput = forwardRef(
  (
    {
      item,
    ...
    const updateItem = (e) => {
      ...
      if (value === '') {
        e.target.value = '0'
        item[name] = 0
        changeNow && changeNow(value, numberProperty)
        return
      }

      item[name] = Number(value.replace(',', ''))
      if (name === 'prra' && item[name] > 100) {
        item[name] = 0
        exeFunc(ref, 'open', '지분율 초과하였습니다.다시 작성 부탁드립니다.')
      }
      // 소수점 1자리 입력
      if (value.indexOf('.') > 1) {
        let resNum = value.split('.')[1].slice(0, 1)

        e.target.value = Number(value.split('.')[0]) + '.' + resNum
      } else {
        if (name === 'invtCareer' || name === 'cnwkYyCnt') {
          e.target.value = item[name]
        } else {
          e.target.value = numberCommaFormat.format(item[name])
        }
    ...
    const inputUi = readOnly ? (
      <CssTextField
        ...
        InputProps={
          ...
          ((type === 'percent' || numberProperty === 'prra') && { endAdornment: <InputAdornment position="end">%</InputAdornment> }) ||
          (numberProperty !== 'prra' && title !== '' && { endAdornment: <InputAdornment position="end">원</InputAdornment> }) ||
          (type === 'hundMilliwon' && { endAdornment: <InputAdornment position="end">억원</InputAdornment> }) ||
          (type === 'fundNum' && {endAdornment: <InputAdornment position="end">개 펀드</InputAdornment>})
        }
      />
    ) : (
      <CssTextField
