...
const NdaWrite = (props) => {
    ...
    const checkStampYn = async () => {
        const res = await ResponseUtils.getSimpleResponse(Api.my.company.seal);
        if(res) {
    ...
    const setEformData = (data) => {
        if (StringUtils.hasLength(data?.dsmsInttSignFile) && StringUtils.hasLength(data?.dsmsInttSignprsnNm)) {
            setVo({
                ...vo,
                ...
                enprNm: commonContext.state.user.info?.name,
                enprSign: data.dsmsInttSignFile,
                enprAdr: data.dsmsInttAdr,
                enprSignprsnNm: data.dsmsInttSignprsnNm,
            });
            setIsSigned(true);
        }
    }
    ...
    const save = async () => {
        const res = await CommonAxios(getPostConfig(Api.my.company.ndaSubmit, vo), false);
    ...
    const autoCloseCallback = () => {
        history.push(ROUTER_NAMES.MY_PAGE_COMPANY_NDA_SEND);
    }

    const loadAuditTargetList = async () => {
        return await ResponseUtils.getList(Api.my.company.ndaTargetList, null, 'list', false);
    }

    const loadRegistNdaFormInttData = async () => {
        const res = await ResponseUtils.getSimpleResponse(Api.my.company.ndaRgistFormInttData, null, false);
        if(res) {
            setVo({
                ...vo
                , enprNm: res.hasOwnProperty('inttNm') ? res.inttNm : ''
                , enprAdr: res.hasOwnProperty('inttAdr') ? res.inttAdr : ''
                , enprSign: res.hasOwnProperty('signData') ? res.signData : ''
                , enprSignprsnNm: res.hasOwnProperty('signprsnNm') ? res.signprsnNm : ''
            });
        }
    }
    ...
    return (
        <>
                ...
                <div className="wrap mypage_wrap nda_wrap write mypage_common bg02">
                    ...
                    <div className="container nda_request_container default_size02">
                        ...
                        <NdaEformPopup ref={ndaEformPopupRef}
                                       signType={NdaSignTypeCode.DSMS_SIGN} usisType={UsisType.COMPANY} vo={vo}
                        ...
                        <CardLayout>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div className="sign_btn_wrap">
                                                    {
                                                        isSigned
                                                            ? <p className="ico_complete" onClick={onClickSign}>서명완료</p>
                                                            : (
                                                                <>
                            ...
                            <div className="btn_group">
                                <Button className={''} onClick={() => {
                                    ...
                                    if(StringUtils.hasLength(type) && type === 'send') history.push(ROUTER_NAMES.MY_PAGE_COMPANY_NDA_SEND);
                                    else history.push(ROUTER_NAMES.MY_PAGE_COMPANY_NDA_RECEIVE);
                                }}>취소</Button>
            ...
            <ConfirmPopup ref={alertConfirmSealRef} onConfirm={() => history.push(ROUTER_NAMES.MY_PAGE_COMPANY_STAMP_WRITE)} />
