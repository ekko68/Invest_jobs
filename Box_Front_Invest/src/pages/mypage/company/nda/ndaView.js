...
const NdaView = (props) => {
    ...
    const setEformData = (data) => {
        if (StringUtils.hasLength(data?.rcvInttSignFile)
            && StringUtils.hasLength(data?.rcvInttSignprsnNm)
            && StringUtils.hasLength(data?.ndaPtdfFileId)) {
            setVo({
                ...vo,
                ...
                enprSign: data.rcvInttSignFile,
                enprAdr: data.rcvInttAdr,
                enprSignprsnNm: data.rcvInttSignprsnNm,
            });
            setIsSigned(true);
        }
    }
    ...
    const onClickList = () => {
        ...
        if(StringUtils.hasLength(query?.type) && query.type === 'send') url = ROUTER_NAMES.MY_PAGE_COMPANY_NDA_SEND;
        else url = ROUTER_NAMES.MY_PAGE_COMPANY_NDA_RECEIVE;
    ...
    const checkStampYn = async () => {
        const res = await ResponseUtils.getSimpleResponse(Api.my.company.seal, null, false);
        if(res) {
    ...
    const save = async () => {
        const res = await CommonAxios(getPostConfig(Api.my.company.ndaContractSign, vo), false);
    ...
    const cancel = async () => {
        const res = await CommonAxios(getPostConfig(Api.my.company.ndaContractCancel, {id: vo.ndaCnttId}), false);
        if (res?.status === 200 && res?.data?.code === '200') {
    ...
    const loadNdaView = async () => {
        ...
        if (StringUtils.hasLength(query?.ndaCnttId)) {
            const url = Api.my.company.ndaContractDetail + "/" + query.ndaCnttId;
            ...
            <div className="page_container">
                <div className="wrap mypage_wrap nda_wrap write mypage_common bg02">
                    ...
                    <div className="container nda_request_container default_size02">
                        ...
                        {/*NDA eform 팝업*/}
                        <NdaEformPopup ref={ndaEformSignPopupRef} signType={NdaSignTypeCode.RCV_SIGN} usisType={UsisType.COMPANY} vo={vo} setEformData={setEformData}/>
                        {/*NDA 서명 확인 eform 팝업*/}
                        <NdaEformPopup ref={ndaEformCompletePopupRef} signType={NdaSignTypeCode.COMPLETE} usisType={UsisType.COMPANY} vo={vo} setEformData={null}/>
                        {/*NDA 최종 PDF 팝업*/}
